Shader "Custom/Geometry Vertex Grass Shader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void geom(triangle v2g IN[3], inout TriangleStream<g2f> triStream) {
                ...
                float3 movementFactor = normalize(_AirVector) * (((sin(_Time * _Speed) + 1) / 2.5) * _AirForce + 0.2);
                for (int j = 0; j < 3; j++) {
                    v2g vertArray[] = IN;
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        o.vertex = UnityObjectToClipPos(vertArray[i].vertex + normal * _ExtrusionFactor + (barycenter - vertArray[i].vertex) * 0.3 + (globalBarycenter - vertArray[i].vertex) * 0.2 + float4(pow(_ExtrusionFactor / 2, 2) * movementFactor, 0.0));
                        UNITY_TRANSFER_FOG(o, o.vertex);
                        o.uv = TRANSFORM_TEX(vertArray[i].uv, _MainTex);
                        ...
                        o.vertex = UnityObjectToClipPos(vertArray[next].vertex + normal * _ExtrusionFactor + (barycenter - vertArray[next].vertex) * 0.3 + (globalBarycenter - vertArray[next].vertex) * 0.2 + float4(pow(_ExtrusionFactor / 2, 2) * movementFactor, 0.0));
                        UNITY_TRANSFER_FOG(o, o.vertex);
                        o.uv = TRANSFORM_TEX(vertArray[i].uv, _MainTex);
                        o.color = _ColorBottom;
                        triStream.Append(o);
                    ...
                    for (int i = 0; i < 3; i++) {
                        vertArray[i].vertex = vertArray[i].vertex + float4(normal, 0.0) * _ExtrusionFactor + (barycenter - vertArray[i].vertex) * 0.3 + (globalBarycenter - vertArray[i].vertex) * 0.2 + float4(pow(_ExtrusionFactor / 2, 2) * movementFactor, 0.0);
                    }
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        o.vertex = UnityObjectToClipPos(barycenter + normal * _ExtrusionFactor + float4(pow(_ExtrusionFactor, 2) * movementFactor, 0.0));
                        UNITY_TRANSFER_FOG(o, o.vertex);
                        o.uv = TRANSFORM_TEX(vertArray[i].uv, _MainTex);
                        o.color = _ColorTip;
                        triStream.Append(o);
